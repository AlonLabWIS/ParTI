...
geneNames <- read.table("geneListExp.list", as.is=T, h=F)[,1]
geneNamesFilt <- read.table("geneNamesAfterExprFiltering.list", as.is=T, h=F)[,1]
...
SSlist <-
    sapply(1:20, function(i) {
        ...
        ## Xrproj <-
        ##     t(apply(Xr, 2, function(x) { x - avgE })) %*%
        ##     as.matrix(dudi2$c1)
