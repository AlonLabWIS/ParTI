...
myFiles <-
    c("LUAD_UCSC/mutEnrichment_discrete_significant.csv",
      ## "SKCM_UCSC/mutEnrichment_discrete_significant.csv",
      ## "UCEC_UCSC/mutEnrichment_discrete_significant.csv",
      "BRCA_UCSC/mutEnrichment_discrete_significant.csv",
      ## "KIRC_UCSC/mutEnrichment_discrete_significant.csv",
      ## "LUSC_UCSC/mutEnrichment_discrete_significant.csv",
      ## "PRAD_UCSC/mutEnrichment_discrete_significant.csv",
      "COAD_UCSC/mutEnrichment_discrete_significant.csv",
#      "GBM_UCSC/mutEnrichment_discrete_significant.csv",
      "LGG_UCSC/mutEnrichment_discrete_significant.csv",
      ## "THCA_UCSC/mutEnrichment_discrete_significant.csv",
      "BLCA_UCSC/mutEnrichment_discrete_significant.csv",
      "LIHC_UCSC/mutEnrichment_discrete_significant.csv");
...
enrichRes <- sapply(myFiles, function(myFile) {
    ...
    myTab <- myTab[grep("=NaN", myTab[,3], invert=T),]
    return(myTab);
})
...
myCats <- names(allMSigDBcat[1:nCateg]);
## MSumMat <- matrix(NA, length(myCats), sum(sapply(allArchNs, length)))
MSumMat <- matrix(0, length(myCats), sum(sapply(allArchNs, length)))
...
MSumMat0 <- MSumMat;
rownames(MSumMat0) <- c();
pdf("MSumMat_mut.pdf", height=15, width=10)
## heatmap(unique(MSumMat0), scale="none", margins=c(1, 30))
heatmap(MSumMat, scale="none", margins=c(5, 7))
dev.off();
...
toHighlight <- c("TCG", "MATRIX", "DOUBLE STRAND BREAK REPAIR",
