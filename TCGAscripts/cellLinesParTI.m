...
%% Clinical features
close all
ParTI_lite(geneExpression, 1, 10, discrAttrNames, ...
    discrAttr, 0, contAttrNames, contAttr, [], 0.15, ...
    strcat(origPath, '/clinicalEnrichment'), arcOrig);

%% MSigDB gene groups
%clear discrAttr contAttr;
close all
ParTI_lite(geneExpression, 1, 10, [], [], ...
    0, GONames, GOExpression, [], 0.15, ...
    strcat(origPath, '/MSigDBenrichment'), arcOrig);

%% Genes highly expression at each archetype
%clear discrAttr contAttr;
close all
ParTI_lite(geneExpression, 1, 10, [], [], ...
    0, geneNames, geneExpression, [], 0.15, ...
    strcat(origPath, '/genesEnrichment'), arcOrig);

%% Repeat, removing a tissue type
tissues = {'lung', 'central nervous system', 'skin', 'large intestine'};
i = 1;
for i = 1:length(tissues)
    myt = upper(tissues{i});
    tmp = discrAttr(:,1);
    sel = find(~strcmp(tmp, myt));
    
    tGeneExpression = geneExpression(sel,:);
    tDiscrAttr = discrAttr(sel,:);
    tContAttr = contAttr(sel,:);
    tGOExpression = GOExpression(sel,:);
    
    close all;
    [arc, arcOrig, ~] = ParTI_lite(tGeneExpression);
    ParTI_lite(tGeneExpression, 1, 10, discrAttrNames, ...
                tDiscrAttr, 0, contAttrNames, tContAttr, [], 0.15, ...
                strcat(origPath, '/clinicalEnrichment_no', myt), arcOrig);
    
    ParTI_lite(tGeneExpression, 1, 10, [], [], ...
                0, GONames, tGOExpression, [], 0.15, ...
                strcat(origPath, '/MSigDBenrichment_no', myt), arcOrig);
end
