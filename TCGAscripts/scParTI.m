...
% columns
geneExpression = dlmread('PCA_cluster5_magic_matlab.csv', ',');
geneExpression = geneExpression(:,2:7);

geneExpression = dlmread('PCA_cluster124_magic_matlab.csv', ',');
geneExpression = geneExpression(:,2:5);

cd ParTI
[arc, arcOrig, pc] = ParTI_lite(geneExpression);
close all
%[arc, arcOrig, pc, errs, pval] = ParTI(geneExpression);
csvwrite(strcat(origPath, '/ParTI_TAMs_arc.csv'), arc)
csvwrite(strcat(origPath, '/ParTI_TAMs_arcOrig.csv'), arcOrig)
csvwrite(strcat(origPath, '/ParTI_TAMs_pc.csv'), pc)

csvwrite(strcat(origPath, '/ParTI_CCs_arc.csv'), arc)
csvwrite(strcat(origPath, '/ParTI_CCs_arcOrig.csv'), arcOrig)
csvwrite(strcat(origPath, '/ParTI_CCs_pc.csv'), pc)
...
% end
selGenes = importdata('selGenes.list');

geneExpressionAll = geneExpression;
geneExpression = geneExpression(:,selGenes);

geneNamesAll = geneNames;
...
ParTI_lite(geneExpression, 1, size(arcOrig,1), [], ...
    [], 0, geneNamesAll, geneExpressionAll, [], binSize, ...
